...
module {:options "-functionSyntax:4"} BinaryOperations {
    ...
    ensures f(g1.unit) == g2.unit
  {
    ...
    == { assert IsRightInverse(g2.bop, g2.inverse, g2.unit); assert g2.unit ==  g2.bop(f(g1.unit), g2.inverse(f(g1.unit))); }
    ...
    == { assert IsHomomorphism(g1.bop, g2.bop, f); assert g2.bop(f(g1.unit), f(g1.unit)) == f(g1.bop(g1.unit, g1.unit)); }
      g2.bop(f(g1.bop(g1.unit, g1.unit)), g2.inverse(f(g1.unit)));
    == { assert IsLeftUnital(g1.bop, g1.unit); assert g1.bop(g1.unit, g1.unit) == g1.unit; }
  ...
  lemma GroupHomomorphismPreservesInverse<S(!new),T(!new)>(g1: Group<S>, g2: Group<T>, f: S -> T)
    requires IsHomomorphism(g1.bop, g2.bop, f)
    ensures forall x :: f(g1.inverse(x)) == g2.inverse(f(x))
  {
    forall x ensures f(g1.inverse(x)) == g2.inverse(f(x)) {
      calc {
        f(g1.inverse(x));
      == { assert IsRightUnital(g2.bop, g2.unit); }
      ...
      == { assert IsLeftInverse(g1.bop, g1.inverse, g1.unit); assert g1.bop(g1.inverse(x), x) == g1.unit; }
