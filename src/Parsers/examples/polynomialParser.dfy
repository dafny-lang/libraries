...
module PolynomialParser {
  import opened P = StringParsers

  // Parser combinators style
  const parser: Parser<Expr>
    := ConcatL(RecursiveMap(map[
      "atom" := RecursiveDef(0, (callback: ParserSelector<Expr>) => 
        Or(ConcatR(
          String("("), ConcatL(
          callback("term"),
          String(")"))),
      ...
      "factor" := RecursiveDef(1, (callback: ParserSelector<Expr>) => 
      ...
      "term" := RecursiveDef(2, (callback: ParserSelector<Expr>) => 
